  ...
  public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      try {
        ...
        if (! deleteLocOK) {
          this.errMessage = "刪除 bas_loc 表格失敗!";
          return false;
        }
        String locUrl = MyInfo.JH_API_URL + "getLocList";
        ApiResponse<List<BasLoc>> locApiResponse = Ion.with(LoginActivity.this)
                .load(locUrl)
        ...
        if (! locInsertOk) {
          this.errMessage = "新增 庫位 表格失敗!";
          return false;
        }

        boolean deleteProcOK = myDao.deleteProcMast();
        if (!deleteLocOK) {
          this.errMessage = "刪除proc_mast表格失敗!";
          return false;
        }

        String procUrl = MyInfo.JH_API_URL + "getProcList";
        ApiResponse<List<ProcMast>> procApiResponse = Ion.with(LoginActivity.this)
                .load(procUrl)
        ...
        if (!procInsertOK) {
          this.errMessage = "新增proc_mast表格失敗";
          return false;
        }
